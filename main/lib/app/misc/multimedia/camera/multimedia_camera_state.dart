...
class _MultimediaCameraState extends State<MultimediaCamera> with WidgetsBindingObserver, TickerProviderStateMixin, RestorationMixin {
  ...
  @override
  void initState() {
    _showVideo = widget.showOnlyVideo || (!widget.showOnlyVideo && !widget.showOnlyPhoto);
    _showPhoto = widget.showOnlyPhoto || (!widget.showOnlyVideo && !widget.showOnlyPhoto);
  ...
  void _updateCameras(List<CameraDescription> cameras) {
    ...
    if(widget.cameraDescriptionUpdated.isNotNull) {
      widget.cameraDescriptionUpdated!(descriptions.toList());
    }
  }
  ...
  void _initializeCamera(CameraDescription description) async {
    ...
    if(_cameras.isNotEmpty) {
      final CameraController cameraController = CameraController(
        description,
        widget.isWeb ? ResolutionPreset.max : ResolutionPreset.high,
  ...
  void _error(String message, {bool useTip = true}) {
    if(widget.onErrorReceived.isNotNull) {
      widget.onErrorReceived!(message, useTip);
    }
  }
  ...
  void _stopRecording() async {
    ...
    final result = await _stopVideoRecording().whenComplete(() {
      setState(() {
        _isFetching = false;
      });
    });
    ...
    if(result.isNotNull && _isFetching.isFalse) {
      result!.copyWith(duration: _videoDuration, isCamera: true);
      _disposeCameraResources();
      
      if(widget.onRecordingCompleted.isNotNull) {
        widget.onRecordingCompleted!(result);
      } else {
  ...
  void _takePhoto() async {
    ...
    if(result.isNotNull) {
      if(widget.onImageTaken.isNotNull) {
        widget.onImageTaken!(result!);
      } else {
        Navigate.back(result: result);
      }
    } else {
  ...
  void _inform(String message) {
    if(widget.onInfoReceived.isNotNull) {
      widget.onInfoReceived!(message);
    }
  }
  ...
  Widget build(BuildContext context) {
    return ViewLayout(
      ...
      config: _isCameraInitialized ? UiConfig(
        systemNavigationBarColor: Colors.transparent,
        statusBarColor: Colors.transparent,
      ) : null,
      extendBehindAppbar: true,
      floaterPosition: widget.floatingPosition ?? 0,
  ...
  @protected
  Color? get staleColor => Theme.of(context).bottomAppBarTheme.color;

  @protected
  Color get activeColor => CommonColors.instance.green;

  @protected
  Color get inActiveColor => CommonColors.instance.lightTheme2;

  @protected
  Color active(bool value) => value ? activeColor : inActiveColor;

  @protected
  Color get commonColor => CommonColors.instance.lightTheme;

  @protected
  Color get activeFlashColor => CommonColors.instance.yellow;

  @protected
  Color get inActiveFlashColor => CommonColors.instance.green;

  @protected
  Color get pausedColor => CommonColors.instance.error;

  @protected
  Color get recordingColor => CommonColors.instance.error;

  @protected
  Color get progressBackgroundColor => Theme.of(context).unselectedWidgetColor;

  @protected
  Animation<Color?> get progressValueColor => AlwaysStoppedAnimation<Color>(CommonColors.instance.error);

  @protected
  Color? get progressColor => null;

  @protected
  StrokeCap? get progressStrokeCap => null;

  @protected
  double get progressStrokeWidth => 4.0;

  @protected
  IconData get cameraIcon => Icons.photo_camera_rounded;

  @protected
  IconData get videoIcon => Icons.radio_button_on_rounded;

  @protected
  double get actionIconSize => Sizing.font(40);
